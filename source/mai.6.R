...
get.table <- function(url) {
  html <- POST(url, body = list(dep = "6", budjet = "Б", form = "О", 
    spec = "Б-12.03.04", listtype = "1"))
  ...
  column.names <- c("number", "name", "summ", "rus", "math", 
    "phys", "achieve", "advantage", "original", "hostel", 
    "whithout.hostel")
  ...
  for (i in 1:length(table$summ)) {
    table$prior[i] <- tail(strsplit(table$summ[i], split = "*")[[1]], 
      2)[1]
  }
  
  for (i in 1:length(table$summ)) {
    table$summ[i] <- paste(strsplit(table$summ[i], split = "*")[[1]][c(1:3)], 
      collapse = "")
  }
